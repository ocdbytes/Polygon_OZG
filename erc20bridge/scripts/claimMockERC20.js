...
async function main() {
    ...
    // console.log('Deposit Axions  :', depositAxions);

    const depositsArray = depositAxions.data.deposits;
    console.log('Deposits : ', depositsArray);
    ...
    for (let i = 0; i < depositsArray.length; i++) {
        ...
        if (currentDeposit.ready_for_claim) {
            ...
            const txn_data = {
                ...
                deposit_cnt: currentDeposit.deposit_cnt,
                main_exit_root: proof.main_exit_root,
                rollup_exit_root: proof.rollup_exit_root,
                orig_net: currentDeposit.orig_net,
                orig_addr: currentDeposit.orig_addr,
                dest_net: currentDeposit.dest_net,
                dest_addr: currentDeposit.dest_addr,
                amount: currentDeposit.amount,
                metadata: currentDeposit.metadata,
            };

            console.log('Claim Message Txn : ', txn_data);
